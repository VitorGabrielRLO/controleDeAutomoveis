package web.controleDeAutomoveis.controller;
...
import web.controleDeAutomoveis.filter.VacinaFilter;
import web.controleDeAutomoveis.model.Saida;
import web.controleDeAutomoveis.model.Status;
import web.controleDeAutomoveis.model.Vacina;
import web.controleDeAutomoveis.notificacao.NotificacaoSweetAlert2;
import web.controleDeAutomoveis.notificacao.TipoNotificaoSweetAlert2;
import web.controleDeAutomoveis.pagination.PageWrapper;
import web.controleDeAutomoveis.repository.LoteRepository;
import web.controleDeAutomoveis.repository.VacinaRepository;
import web.controleDeAutomoveis.service.LoteService;
...
public class LoteController {
    ...
    public String abrirCadastro(@PathVariable("codigo") Long codigo,
            Model model, HttpSession sessao, Saida lote) {
    ...
    public String cadastrar(@Valid Saida lote, BindingResult resultado,
    ...
    public String pesquisar(LoteFilter filtro, Model model,
            @PageableDefault(size = 7) @SortDefault(sort = "codigo", direction = Sort.Direction.ASC) Pageable pageable,
            HttpServletRequest request) {
        Page<Saida> pagina = loteRepository.pesquisar(filtro, pageable);
        logger.info("Lotes pesquisados: {}", pagina);
        PageWrapper<Saida> paginaWrapper = new PageWrapper<>(pagina, request);
        model.addAttribute("pagina", paginaWrapper);
    ...
    public String abrirAlterar(@PathVariable("codigo") Long codigo, Model model) {
        Saida lote = loteRepository.findByCodigoAndStatus(codigo, Status.ATIVO);
        if (lote != null) {
    ...
    public String alterarHTMX(@Valid Saida lote, BindingResult resultado,
        ...
        if (resultado.hasErrors()) {
            ...
            redirectAttributes.addFlashAttribute("notificacao", new NotificacaoSweetAlert2("Lote alterado com sucesso!", TipoNotificaoSweetAlert2.SUCCESS, 4000));  // redirect
            return "redirect:/lotes/abrirpesquisar";
        }
    }
}
